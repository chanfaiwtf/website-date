...
$query1 = "SELECT sex FROM basic WHERE user_name=$username";
...
while($row2 = mysql_fetch_array($rs2)){
	$txt = $row2['user_name'];
	fwrite($myfile,$row2['user_name']." ");
	if($row2['sex']=="Male"){
		fwrite($myfile,"Female"." ");
	}else{
		fwrite($myfile,"Male"." ");
	}
	fwrite($myfile,$row2['sex']." ");
...
fclose($myfile3);
?>
<meta http-equiv = "refresh" content = "0;url = index-pair-result.php">
